...
public class WavKeysoundWriter implements KeysoundWriter {
	private String keysoundFolderPath;

	public WavKeysoundWriter(String keysoundFolderPath) {
		this.keysoundFolderPath = keysoundFolderPath;

		File folder = new File(keysoundFolderPath);
		folder.mkdir();
	}

	@Override
	public void writeKeysound(String filename, byte[] data,
			StreamInfo streamInfo) throws IOException {

		FileOutputStream os = new FileOutputStream(keysoundFolderPath
				+ filename + ".wav");
		try {
